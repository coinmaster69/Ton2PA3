...
def auto_gain_staging(
    tracks: List[np.ndarray], target_rms_dbfs: float = -20.0
...
def analyse_project(
    ...
    i, j = ref_pair
    energy_pair = energy(tracks[i] + tracks[j])
    rho = correlation(tracks[i], tracks[j])

    raw_mix = np.sum(np.stack(tracks), axis=0)
    raw_rms = rms(raw_mix)
    raw_energy = energy(raw_mix)

    n_tracks = len(tracks)
    delta_L = 10 * np.log10(1 / n_tracks)

    print("\nEinzelspur-Analyse:")
    for idx, (E, r, c) in enumerate(zip(energies, rms_vals, crest_vals)):
        print(
            f"Track {idx}: Energie={E:.2e}, RMS={20*np.log10(r):.2f} dBFS, C={c:.2f}"
        )
    print(f"\nKorrelationsfaktor Track {i}/{j}: {rho:.3f}")
    print(f"Kombinierte Energie Track {i}+{j}: {energy_pair:.2e}")
    print(f"\nRMS Roh-Summe:     {20*np.log10(raw_rms):.2f} dBFS")
    print(f"Energie Roh-Summe: {raw_energy:.2e}")
    print(f"ΔL bei {n_tracks} Spuren: {delta_L:.2f} dB")

    scaled, gains_db = auto_gain_staging(tracks, target_rms_dbfs)
    mix = np.sum(np.stack(scaled), axis=0)
    mix_rms = rms(mix)
    mix_energy = energy(mix)

    print(
        f"\nNach Gain-Staging: RMS = {20*np.log10(mix_rms):.2f} dBFS "
        f"(Ziel {target_rms_dbfs} dBFS)"
    )
    print(f"Energie nach Pegeln: {mix_energy:.2e}")
    ...
    for idx, (E, r, c, g) in enumerate(zip(energies, rms_vals, crest_vals, gains_db)):
        report.append(
            f"Track {idx}: Energie={E:.2e}, RMS={20*np.log10(r):.2f} dBFS, "
            f"C={c:.2f}, Gain={g:+.2f} dB\n"
        )
    report.extend(
        [
            f"\nKorrelationsfaktor {i}/{j}: {rho:.3f}\n",
            f"Kombinierte Energie {i}+{j}: {energy_pair:.2e}\n",
            f"RMS Roh-Summe: {20*np.log10(raw_rms):.2f} dBFS\n",
            f"Energie Roh-Summe: {raw_energy:.2e}\n",
            f"ΔL bei {n_tracks} Spuren: {delta_L:.2f} dB\n",
            f"RMS nach Pegeln: {20*np.log10(mix_rms):.2f} dBFS "
            f"(Ziel {target_rms_dbfs} dBFS)\n",
            f"Energie nach Pegeln: {mix_energy:.2e}\n",
        ]
    )
    rpt_path = save_file(
        "".join(report), out_dir / "pegel_report.txt", None, save_mode
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sr", type=int, default=48_000, help="Samplerate [Hz]")
